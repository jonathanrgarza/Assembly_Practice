@echo off
REM Build script for NASM assembly files without the use of C libraries
...
if "%BuildExt%"=="exe" (
    set BinLinkerFlagsMSVC=/subsystem:console /entry:_start
) else (
    set BinLinkerFlagsMSVC=/dll
)

set CommonLinkerFlagsMSVC=%BinLinkerFlagsMSVC% /nodefaultlib Kernel32.lib Shell32.lib /nologo /incremental:no
